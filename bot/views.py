...
def start_handler(message):
    try:    
        bot.send_video(message.chat.id, 'BAACAgIAAxkBAAIG3GeZt0DJk5hBsnF1mjewzzUEVzVeAALeYAAC--KISIY9zwFY-iTtNgQ', 
...
def retrieve_region(call):
    try:
        ...
        bot.edit_message_text(chat_id=call.from_user.id, message_id=call.message.message_id, reply_markup=None,
                              text='📝Arizangiz haqida batafsil ma\'lumot kiriting\n(Misol: O\'rikzor 5-7 dokondan olingan lyustra karobkadan zapchastida muammo bilan chiqdi)', parse_mode='html')
